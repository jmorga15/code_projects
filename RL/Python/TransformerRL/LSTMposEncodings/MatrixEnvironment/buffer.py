...
class ReplayBuffer():
    def __init__(self, max_size, input_shape, n_actions, alpha=0.99):
        ...
        self.mask_memory = np.zeros((self.mem_size, input_shape))

        self.new_state_memory = np.zeros((self.mem_size, input_shape))
        self.next_mask_memory = np.zeros((self.mem_size, input_shape))
    ...
    def store_transition(self, state, mask, action, reward, state_, mask_, done):
    ...
    def sample_buffer(self, batch_size, beta=0.2):
        ...
        return states, masks, actions, rewards, states_, masks_, dones, indices, weights
